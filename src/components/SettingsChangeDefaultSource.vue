	...
	<div id="app">
		...
		<body class="dashboard">
			...
			<!--  -->
				...
				<div class="payment-cards" v-for="source in this.customerSources" v-bind:key="source.id" v-on:click="promptForDefaultSourceConfirm(source.id)">
					<pre>{{ source.brand }} ending in	{{ source.last4 }} </pre> <pre> expires : {{ source.exp_month }} / {{ source.exp_year }}</pre>
				</div>
            ...
            <div class="container" style="margin:75px">
                <div class="row">
                    <div class="col-md-4">
						...
						<h4>
							Would you like to make this card your default payment method ?
						</h4>
...
export default {
	...
	async makeDefaultSource() {
		try {
			...
			const response = await SettingsService.updateDefaultSource(updateDefaultSourceInfo)
    ...
    async getStripeUserInfo () {
      try {
        const userExtracted = this.$store.getters.getUserInfo
        const stripeUserInfo = await SettingsService.getStripeUserInfo(userExtracted)
		...
		console.log(`\nThe customer's default source is : ${JSON.stringify(stripeUserInfo.data.stripeCustomer.default_source)}\n`) // TESTING
      } catch (error) {
